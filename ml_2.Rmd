...
# cache = TRUE so it doesn't run every time. 
...
```{r how good is this model?}

# kappa is you did this percent better than a random guess
...
```{r make a better model and test it}
...
```{r unncessary variables?}

model_3 <- glm(data = clean, dvote ~ gender + party, family = "binomial")

x_3 <- clean %>% 
  mutate(prediction = predict(model_3, type = "response")) %>% 
  mutate(pred_dvote = as.factor(ifelse(prediction > mean(prediction), "Yes", "No")))

metrics(data = x_3, truth = dvote, estimate = pred_dvote) %>% 
  gt() %>% 
